...
<?php
    $total_price=0;
        ...
        foreach($cartid as $id){
            $productIncart=$cart->getInfoCartByCartID($id);
            if($productIncart!=0){
                $infoProduct=$product->getInfoProduct($productIncart[0]['pid']);
                $total_price+=$productIncart[0]["pprice"]*$productIncart[0]["cartquantity"]*(100-$infoProduct['pdiscount'])/100;
            }
            
        }
    }
    else{
        $infoProduct=$product->getInfoProduct($_SESSION['pid']);
        $total_price=$infoProduct['pprice']*(100-$infoProduct['pdiscount'])*$_SESSION['quantity']/100;
    }    
?>

<body>
    <?php require_once("View/vnpay_php/config.php"); ?>
    <div class="container">
